"""
FFA-Net core library: model, metrics, dataset, training and inference utilities.
"""
...
def build_dataloaders(data_dir, trainset, testset, batch_size, crop, crop_size):
    """Build train and test DataLoaders for RESIDE datasets."""
    ...
    return loader_map[trainset](), loader_map[testset]()
...
def evaluate(net, loader_test, device):
    """Evaluate on test set; returns (mean_ssim, mean_psnr)."""
    net.eval()
...
def train(args):
    ...
    print(f"Device: {device}  |  Checkpoint: {model_path}")
    ...
    loader_train, loader_test = build_dataloaders(
        args.data_dir, args.trainset, args.testset, args.bs, args.crop, args.crop_size,
    )
    ...
    if args.resume and os.path.exists(model_path):
        ...
        losses, start_step = ckp["losses"], ckp["step"]
        max_ssim, max_psnr = ckp["max_ssim"], ckp["max_psnr"]
        psnrs_hist, ssims_hist = ckp["psnrs"], ckp["ssims"]
    ...
    for step in range(start_step + 1, T + 1):
        ...
        if step % args.eval_step == 0:
            ...
            if ssim_eval > max_ssim and psnr_eval > max_psnr:
                max_ssim, max_psnr = ssim_eval, psnr_eval
                torch.save(
                    {"step": step, "max_psnr": max_psnr, "max_ssim": max_ssim,
                     "ssims": ssims_hist, "psnrs": psnrs_hist, "losses": losses,
...
def test(args):
    """Inference on a folder of hazy images."""
    device = "cuda" if torch.cuda.is_available() else "cpu"
    ...
    net = nn.DataParallel(FFA(gps=args.gps, blocks=args.blocks))
    net.load_state_dict(ckp["model"])
    net = net.to(device)
    net.eval()
    ...
    for im_name in img_files:
        ...
        vutils.save_image(torch.squeeze(pred.clamp(0, 1).cpu()), os.path.join(args.output, out_name))
